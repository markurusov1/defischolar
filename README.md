...
Troubleshooting
- Module/theme isn’t loading: run `hugo mod get -u` and ensure Go is installed and on PATH.
- CSS/JS not updating: stop the dev server and restart `hugo server -D` to clear caches.
- Broken internal links on GitHub Pages: confirm `baseURL` in `hugo.toml` matches the live domain.
